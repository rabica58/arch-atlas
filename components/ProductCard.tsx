...
export default function ProductCard(props: Props) {
  ...
  return (
    ...
    >
      ...
      >
        {admin ? null : (
          <TbShoppingBagPlus className="absolute right-1/2 top-1/2 rounded-full opacity-0 transition-all group-hover:opacity-100 z-[1] text-3xl" onClick={() => addToCart({ ...product, count: 1  })} />
        )}
        <Image
          src={product.image}
          alt="Image"
      ...
      <p className="font-bold leading-none tracking-tighter text-lg text-primary/80">{product.title}</p>
      <p className="leading-none mb-6">{product.price}$</p>
    </Link>
  );
}
